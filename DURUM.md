...
**Son güncelleme:** 2026-09-21 (+03) — **SINIRLI KAPANIŞ TAMAM:** ASF meta uyarı notu + eşleştirmesiz güç kontrolü → metatranskriptom kolu RESMEN KAPATILDI + meta kolu ana mimari DAG'ına ENTEGRE + Pages. Detay: bellekte [[reminder_rnaforge_asf_metatx]].

## Şu an nerede kaldık (2026-09-21 en güncel)
- **★ METATRANSKRİPTOM KOLU RESMEN KAPATILDI (2026-09-21).** Eşleştirmesiz güç kontrolü koşuldu (`config/asf_nod_unpaired.yaml`, `de.design:"~condition"` + `paired:false`, run `runs/20260921_104206_asf_nod_unpaired`). **Sonuç: eşleştirmeyi kaldırmak gücü ARTIRMADI, düşürdü** — en küçük padj eşleştirilmiş 0.110 → eşleştirmesiz 0.296 (aynı gen C825_RS08160/ASF519, log2FC ~+6.5→+7.0 kolon; her ikisinde de padj<0.05 = 0 DEG). Biyolojik yorum: sekum/kolon farkı fare-içinde tutarlı → eşleştirilmiş `~subject+condition` zaten daha iyiydi; eşleştirmesiz +1 df kazandırsa da varyans ekleyip sinyali zayıflattı. **KARAR: kol YETERLİ kabul edildi** — mekanik + istatistiksel + null-uyumu SAĞLAM; eksik "güçlü-pozitif konkordans" mimari/tasarım kusuru DEĞİL, n=3 + 2-takson baskınlığından kaynaklı güç sınırı (yeni-veri arama borcu KALDIRILDI, kullanıcı kararıyla). Her iki rapor da meta uyarı notunu içeriyor.
- **★ META KOLU ANA MİMARİ DAG'INA ENTEGRE (2026-09-21).** `PipelineForge/specs/rnaforge.yml` üçüncü organizma dalı olarak metatranskriptomu kazandı: organizma kararı 3-yollu (prokaryot sol · **metatranskriptom merkez omurga: rRNA→m04m→m05m + TAX** · ökaryot sağ), hepsi m06 hub'ında birleşir; yeni `rose` kategori rengi (forge.py'ye 5. renk eklendi), 6-kolon layout, viewBox 1320×1330, adaptif wrap genişliği. `docs/pipeline_architecture.html` yeniden üretildi (25 kart/28 kenar, self-validate ✓). GÖRSEL QA: chrome-headless-shell PNG ile TR sayfa + renk ayrımı doğrulandı (temiz, sığıyor). README'lere metatranskriptom DAG bağlantısı eklendi.
...
- **★★★ METATRANSKRİPTOM KOLU — main'e MERGE'li + GERÇEK-VERİ DOĞRULAMASI (2026-08-20).** 12 görevlik kol (TDD, 680 test)
  ...
  - **B. theta VPI-5482 gnotobiyotik (PRJEB48101, Weagley Cell Reports 2022, NAD-deficient vs NA-supplemented,
    ...
    - ❌ **GÜÇLÜ POZİTİF BİYOLOJİK HİKAYE YOK:** NAD homeostaz genleri (nadE `BT_RS01010` padj 0.61, nadC `BT_RS07890` padj 0.99)
      anlamsız; 50 DEG çoğu hypothetical/DUF/besin geni. "NAD genleri UP" bir BEKLENTİYDİ, makale bulgusu değildi (makale null).
      Yani seçtiğimiz kontrast güçlü-pozitif DE için uygun DEĞİLDİ (makale de null bulmuş). Referanslar `references/btheta` +
      `references/kraken2/btheta` gitignore'lu diskte; config `config/btheta_nad.yaml`+`.samples.tsv` commit'li.
  - **DÜRÜST DURUM:** Kol mekanik + istatistiksel + null-uyumu olarak SAĞLAM; downstream (m06+) zaten 5 kolda güçlü doğrulanmış.
    Eksik olan tek şey diğer 5 kol gibi bir "güçlü-pozitif biyolojik konkordans" örneği — iki denemede de veri/kontrast (kod değil)
    nedeniyle ulaşılamadı. Ders: metatranskriptom set seçiminde DE-gücü makaleden ÖNCE teyit edilmeli.
